...
int adjust_volume(int maxv, double t, int notelength) ;
int p1(int maxv, double t, int nl) ;
int p2(int maxv, double t, int nl) ;
int p3(int maxv) ;
int p4(int maxv, double t, int nl) ;
...
int adjust_volume(int maxv, double t, int notelength) {
	if (t < notelength/6) {
		return p1(maxv, t, notelength) ;
	}else if (t < notelength/3) {
		return p2(maxv, t, notelength) ;
	}else if (t < 5*notelength/6) {
		return p3(maxv) ;
	}else return p4(maxv, t, notelength) ;
}


int p1(int maxv, double t, int nl) {	//first equation of piecewise function for adjust volume
	double yint = maxv/3 ;
	double m = 4 * maxv / nl ;
	return m*t + yint ;
}
